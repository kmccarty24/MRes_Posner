...
emphasis = quote.upper()

# 2.4 Below this line, alter the the code to include the newly emphasised quote 

interviewQuote = "I laughed so much when we encountered" + emphasis

print interviewQuote
...
string = 'First Name: %s \t Surname: %s \t DOB: %s \n First Name: %s \t Surname: %s \t DOB: %s' %(first, sur, DOB, first2, sur2, DOB2)
...
p_info = {'p': 100, 'age': 55, 'occupation': 'brick layer'} 
...
# 7.2 Using the list below, create an if statement which compares whether a name that you input 
...
if nameTest in names:
    print 'present'
elif nameTest not in names:
    print 'not in list'
...
while whileCounter < 1:
    if whileCounter > 300:
        print 'Huge number'
        whileCounter -= 5
    elif whileCounter <= 300 and whileCounter > 200:
        print 'getting smaller'
        whileCounter -= 5
    elif whileCounter<= 200 and whileCounter >= 100:
        print 'This is tiresome'
        whileCounter -= 5
    elif whileCounter <= 999:
        print 'Bored'
        break
...
def pCo(MMM, name, DOB):
    code = '%s%s%s' %(MMM[:2], DOB[:2], name[:2])
    return code
...
pCode = pCo(MMM = MMM, name = name, DOB = dateOfBirth)
